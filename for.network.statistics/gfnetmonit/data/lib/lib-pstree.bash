...
#=============================================================================={
##    Destiny:    Lib for Graphical User Interface for " netmonit --catch-all-with-lsof "
     VERSION="2" 
...
#           The newer version of pstree should already be corrected.
        ...
        #--------------------------}
        Source_If_Exist lib-2-colorful-messages.bash
        #---------------------------------------------}


PID="$1"
Ps_Tree=$(cat /var/log/netmonit/${PID}.pstree)
...
while IFS=',' read -r COLUMNS_AND_NAME  PID_AND_ARGUMENTS ; do
	...
	while IFS=' ' read -r PID_LINE OPTIONS ; do
		...
		## Remove glued arguments from number
		PID_LINE=$( grep -P -o "[0-9]+" <<< "$PID_LINE")
